...
public class TrimPatternGenerator implements DataProvider {
    ...
    public TrimPatternGenerator(DataOutput output) {
    ...
    public CompletableFuture<?> run(DataWriter writer) {
        ...
        return jsons.entrySet().stream().map(entry -> DataProvider.writeToPath(writer, entry.getValue(), resolver.resolveJson(entry.getKey()))).collect(futureCollector());
    }
